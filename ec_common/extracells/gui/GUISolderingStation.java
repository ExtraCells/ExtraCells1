...
public class GUISolderingStation extends GuiScreen {
    ...
    public void updateScreen() {
        if (this.mc.thePlayer.getHeldItem() != null) {
            if (this.mc.thePlayer.getHeldItem().getItem() == extracells.Cell
                    && this.mc.thePlayer.getHeldItem().getTagCompound()
                            .getString("SubItemName").equals("adjustable")) {
                if (this.mc.thePlayer.getHeldItem().hasTagCompound()) {
                    PacketDispatcher.sendPacketToServer(new SolderingPacket(
                            mc.thePlayer.username, tileX, tileY, tileZ,
                            int_size, int_types, false).makePacket());
                    ItemStack itemstack = this.mc.thePlayer.getHeldItem();
                    itemstack.getTagCompound().setInteger("costum_size",
                            int_size);
                    itemstack.getTagCompound().setInteger("costum_types",
                            int_types);

                    if (textfield_size.getText() != Integer.toString(itemstack
                            .getTagCompound().getInteger("costum_size"))
                            || textfield_types.getText() != Integer
                                    .toString(itemstack.getTagCompound()
                                            .getInteger("costum_types"))) {
                        PacketDispatcher
                                .sendPacketToServer(new SolderingPacket(
                                        mc.thePlayer.username, tileX, tileY,
                                        tileZ, int_size, int_types, false)
                                        .makePacket());
        ...
        switch (button.id) {
            case 0:
                ...
                if (Util.getCellRegistry()
                    ...
                    if (!((int_size - 2048) < 4096)) {
                        if (isSpaceInInventory(appeng.api.Materials.matStorageCell
                                .copy())) {
                            PacketDispatcher
                                    .sendPacketToServer(new SolderingPacket(
                                            mc.thePlayer.username, tileX,
                                            tileY, tileZ, int_size, int_types,
                                            false).makePacket());
                            this.mc.thePlayer.inventory
                                    .addItemStackToInventory(new ItemStack(
                                            appeng.api.Materials.matStorageCell
                                                    .copy().getItem(), 1,
                                            appeng.api.Materials.matStorageCell
                                                    .getItemDamage()));
                            ...
                            this.mc.thePlayer
                                    .sendChatToPlayer("Make place in your inventory to regain the ME Storage Cell!");
                        }
                    }
                } else {
                    this.mc.thePlayer
            ...
            case 1:
                ...
                if (Util.getCellRegistry()
                        .getHandlerForCell(
                                this.mc.thePlayer.getCurrentEquippedItem())
                        .storedItemCount() == 0) {
                    if (doesInvContain(appeng.api.Materials.matStorageCell
                            .copy())) {
                        PacketDispatcher
                                .sendPacketToServer(new SolderingPacket(
                                        mc.thePlayer.username, tileX, tileY,
                                        ...
                                        .makePacket());
                        decreaseStackInInv(
                                appeng.api.Materials.matStorageCell.copy(), 1);
                        int_size = int_size + 2048;
                    } else {
                        this.mc.thePlayer
                                .sendChatToPlayer("To upgrade the size of your storage, you need one ME Storage Cell for each 2048 KBytes! Thats double the size per cell you'd get with 1K Storages :D");
                    }
                } else {
                    this.mc.thePlayer
                            .sendChatToPlayer("This Cell is not Empty!");
            ...
            case 2:
                ...
                if (Util.getCellRegistry()
                        ...
                        .storedItemCount() == 0) {
                    if (!((int_types - 1) < 27)) {
                        if (isSpaceInInventory(appeng.api.Materials.matConversionMatrix
                                .copy())) {
                            PacketDispatcher
                                    .sendPacketToServer(new SolderingPacket(
                                            mc.thePlayer.username, tileX,
                            ...
                            this.mc.thePlayer.inventory
                                    .addItemStackToInventory(new ItemStack(
                                            appeng.api.Materials.matConversionMatrix
                                                    .copy().getItem(),
                                            1,
                                            appeng.api.Materials.matConversionMatrix
                                                    .getItemDamage()));
                            ...
                            this.mc.thePlayer
                                    .sendChatToPlayer("To upgrade the number of types of your storage, you need one ME Conversion Matrices for each type!");
                        }
                    }
                } else {
                    this.mc.thePlayer
    ...
    public boolean doesInvContain(ItemStack itemstack) {
        for (int i = 0; i < this.mc.thePlayer.inventory.mainInventory.length; i++) {
            if (this.mc.thePlayer.inventory.mainInventory[i] != null) {
                if (this.mc.thePlayer.inventory.mainInventory[i].getItem() == itemstack
                        .getItem()
                        && this.mc.thePlayer.inventory.mainInventory[i]
        ...
        for (int i = 0; i < this.mc.thePlayer.inventory.mainInventory.length; i++) {
            if (this.mc.thePlayer.inventory.mainInventory[i] != null) {
                if (this.mc.thePlayer.inventory.mainInventory[i].getItem() == itemstack
                        ...
                        && this.mc.thePlayer.inventory.mainInventory[i]
                                .getItemDamage() == itemstack.getItemDamage()
                        && this.mc.thePlayer.inventory.mainInventory[i].stackSize <= 63) {
                    return true;
                }
            } else {
                return true;
                ...
                if (this.mc.thePlayer.inventory.mainInventory[i].getItem() == itemstack
                    ...
                    if (left != 0) {
                        if (this.mc.thePlayer.inventory.mainInventory[i].stackSize == 1) {
                            ...
                            this.mc.thePlayer.inventory.mainInventory[i] = new ItemStack(
                                    this.mc.thePlayer.inventory.mainInventory[i]
                                            .getItem(),
